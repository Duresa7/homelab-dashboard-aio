// UniFi Protect integration. Three concerns:
//  1. Camera/NVR snapshot data (the `protect` slice), cached.
//  2. A persistent WebSocket event subscriber feeding an in-memory ring buffer.
...
async function fetchProtectDataRaw() {
...
const fetchProtectData = withTtlCache(fetchProtectDataRaw, PROTECT_CACHE_TTL);
...
export function shutdownProtect() {
  shuttingDown = true;
...
// Status descriptor consumed by /api/health + startup logging in index.js.
export const protectStatus = {
  enabled: PROTECT_ENABLED,
  configured: !!(PROTECT_BASE_URL && PROTECT_API_KEY),
  baseUrl: PROTECT_BASE_URL,
};

/** Liveness probe used by /api/health/live. */
export function probeProtect() {
  return protectFetchJson('/v1/meta/info');
}

/** Kick off background work (ffmpeg detection + the WS subscriber) once at boot. */
export function startProtect() {
  ...
  app.get('/api/protect/debug', async (_req, res) => {
    ...
    res.json({
      ...
      cache: c.data
        ? {
            ageMs: Date.now() - c.ts,
            cameras: c.data.protect.cameras.length,
            connected: c.data.protect.connected,
            nvr: c.data.protect.nvr?.name || null,
          }
        : null,
      events: {
        connected: !!protectWs && !!protectWsConnectedAt,
